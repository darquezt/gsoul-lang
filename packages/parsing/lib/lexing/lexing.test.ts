...
describe('Lexing types', () => {
  ...
  test('A sensitivity environment', () => {
    expect(scanTokens('@[2x + 3y]')).toStrictEqual<Token[]>(
      withEOF([
        new Token(TokenType.AT, '@', null, 1, 1),
        new Token(TokenType.LEFT_BRACKET, '[', null, 1, 2),
        new Token(TokenType.NUMBERLIT, '2', 2, 1, 3),
        new Token(TokenType.IDENTIFIER, 'x', null, 1, 4),
        new Token(TokenType.PLUS, '+', null, 1, 6),
        new Token(TokenType.NUMBERLIT, '3', 3, 1, 8),
        new Token(TokenType.IDENTIFIER, 'y', null, 1, 9),
        new Token(TokenType.RIGHT_BRACKET, ']', null, 1, 10),
      ]),
    );
  });
  ...
  test('An arrow type', () => {
    expect(scanTokens('(x:Number -> Bool@[])')).toStrictEqual<Token[]>(
      withEOF([
        ...
        new Token(TokenType.IDENTIFIER, 'x', null, 1, 2),
        new Token(TokenType.COLON, ':', null, 1, 3),
        new Token(TokenType.NUMBER, 'Number', null, 1, 4),
        new Token(TokenType.ARROW, '->', null, 1, 11),
        new Token(TokenType.BOOL, 'Bool', null, 1, 14),
        new Token(TokenType.AT, '@', null, 1, 18),
  ...
  test('A function', () => {
    expect(scanTokens('fun(x:Number) {x;}')).toStrictEqual<Token[]>(
      withEOF([
        ...
        new Token(TokenType.COLON, ':', null, 1, 6),
        new Token(TokenType.NUMBER, 'Number', null, 1, 7),
        new Token(TokenType.RIGHT_PAREN, ')', null, 1, 13),
        new Token(TokenType.LEFT_BRACE, '{', null, 1, 15),
        new Token(TokenType.IDENTIFIER, 'x', null, 1, 16),
        new Token(TokenType.SEMICOLON, ';', null, 1, 17),
        new Token(TokenType.RIGHT_BRACE, '}', null, 1, 18),
      ]),
    );
  });
});
  ...
  test('An ascription', () => {
    expect(scanTokens('x :: Number@[]')).toStrictEqual<Token[]>(
      withEOF([
        ...
        new Token(TokenType.AT, '@', null, 1, 12),
